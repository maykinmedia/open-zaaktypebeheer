...
import {Body, Card, ErrorMessage, Form, FormControl, H1, Outline, Toolbar} from '@maykin-ui/admin-ui/components';
...
const DashboardView = () => {
    ...
    const filteredZaaktypen = attributeOnQueryFilter(query, zaaktypen, 'omschrijving').map(
        (z) => {
    ...
    return (
        <>
            <List
                ...
                loading={isLoading}
                results={filteredZaaktypen as unknown as AttributeData<Attribute>[]} sort={true}
            ...
            >
                <Card>
                    <Body>
                        {errorCatalogi && (
                            <ErrorMessage>
                                Er is een fout opgetreden bij het ophalen van de catalogi. Probeer
                                het opnieuw.
                            </ErrorMessage>
                        ...
                        <Toolbar align="space-between">
                            <H1>Zaaktypen&nbsp;{isLoading &&
                                <Outline.ArrowPathIcon spin={true} aria-label={"Bezig met laden"}/>}</H1>
                            <Form autoComplete="off" direction="horizontal" showActions={false}>
                                <FormControl
